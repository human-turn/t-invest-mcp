...
(доли в процентах; goals — массив, целей может быть несколько).
Если файла нет — покажи текущую структуру, предложи значения и, после согласования с пользователем, создай файл.`;
...
5. Отметь, что из этого уже есть в моём портфеле (get_portfolio). Это скрининг, не рекомендация.`,
  ...
  investCash: (amount: string) => `Пришло пополнение ${amount} ₽ — распредели его докупками, приближая портфель к целевым долям (БЕЗ продаж).

1. ${TARGET_FILE_CONVENTION}
2. get_portfolio → текущие доли; посчитай недовес каждого класса/тикера относительно целей в рублях.
3. Распредели ${amount} ₽ по недовешенным позициям пропорционально недовесу, в ЦЕЛЫХ ЛОТАХ (lot и цены из get_instrument/get_last_prices; облигации — цена в % номинала + НКД!).
...
export function registerPrompts(server: McpServer): void {
  ...
  server.registerPrompt(
    "invest_cash",
    {
      ...
      argsSchema: { amount: z.string().describe("Сумма пополнения в рублях, например 100000") },
    },
  ...
  server.registerPrompt(
    "weekly",
    {
      ...
      argsSchema: {
        amount: z.string().optional().describe("Сумма пополнения ₽ (без неё раздел докупки пропускается)"),
      },
    },
    ({ amount }) =>
      text(
