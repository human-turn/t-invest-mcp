...
<!-- TODO(vhs): короткое демо возможностей — docs/demo.gif -->
<!-- ![demo](docs/demo.gif) -->

```text
Ты:    покажи портфель — что просело, что выросло?
Claude: [get_portfolio] Портфель 2.43 млн ₽ (+11.8%). Просели: GAZP −4.2%…

Ты:    какие купоны и дивиденды придут летом?
Claude: [get_bond_coupons, get_dividends] Июнь: 12 400 ₽ (ОФЗ 26248, SBER)…

Ты:    выгрузи 5 лет свечей SBER и построй график со скользящими
Claude: [get_candles → data/sber.csv, 1353 строки] Строю график…
```
...
появляются как slash-команды `/mcp__t-invest__<имя>`:
...
### Нашли проблему? `/mcp__t-invest__feedback`
...
- **Данные — в свои руки.** Каждый инструмент умеет выгружать результат в файл
  ...
  забирает минутки целыми годами (1 запрос = 1 год, нужен боевой токен).
  Дальше — любая своя аналитика: бэктесты, графики, отчёты.
